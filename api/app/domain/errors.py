...
class ErrUnexpected(ApiException):
    """
...
class ErrNonBlockingUnexpected(ApiException):
    """
    ErrUnexpected represents global exception for errors that should not happen
    and we don't wish to provide information to end user.
    """
...
class ErrUnauthorized(ApiException):
    """
...
class ErrTooManyRequest(ApiException):
    """
...
class ErrNotFound(ApiException):
    """
...
class ErrInvalidData(ApiException):
    """
...
errors_priority = {
    0: (ErrInvalidData, ErrNotFound),
    1: (ErrUnauthorized, ErrInvalidResponseFormat, ErrTooManyRequest),
}
